...
def pos2feature(pos,etaL,kappa,etaE,v,fakeEnergy=10000):
    num_of_atom=len(pos[0]) #pos是n个构型
    feature=[]
    for a_cluster in pos:
        line=[num_of_atom]
        t=list(_flatten(a_cluster))
        line=line+t
        line=line+[fakeEnergy]
        tempCluster=cluster(line)
        tempFeature=cluster2fetureLabel(tempCluster,etaL,kappa,etaE,v,1,1,1,1)
        feature.append(tempFeature[0])
    feature=np.array(feature,dtype='float64')
    return feature

def cluster2fetureLabel (clusterIn,etaE,kappa,etaL,v,A_matByE,L_matByE,A_matByL,L_matByL):
    feature = DisMat2featureVector(clusterIn.dis_mat,etaE=etaE,kappa=kappa,etaL=etaL,v=v,A_matByE=A_matByE,L_matByE=L_matByE,A_matByL=A_matByL,L_matByL=L_matByL)
    label = clusterIn.energy
