...
epo=1000# training epoch
batc=128

def train_GBR_model(feature_set, label_set,n_estimators, subsample, min_samples_split, max_depth,seed,n_iter_no_change,continueTrain=False): #计划加入一个savedir参数保存模型
    #input:
    #   training data; should be numpy
    #output: 
    #   a model after training
    print(feature_set.shape)
    model=GBR(loss='ls',learning_rate=0.001,n_iter_no_change=n_iter_no_change,n_estimators=n_estimators,max_depth=max_depth,min_samples_split=min_samples_split,subsample=subsample,max_features='sqrt',random_state=seed)
...
def train_SVR_model(feature_set,label_set,tol,epsilon,kernel,C,seed):
    print("start training SVR model")
    model=SVR(verbose=False,tol=tol,epsilon=epsilon,kernel=kernel,C=C)
